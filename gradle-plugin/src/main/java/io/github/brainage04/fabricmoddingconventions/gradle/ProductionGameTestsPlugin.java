...
public final class ProductionGameTestsPlugin implements Plugin<Project> {
    ...
    private static TaskProvider<? extends Task> registerClientProductionGameTest(
        ...
        return project.getTasks().register("runProductionClientGameTest", ClientProductionRunTask.class, task -> {
            ...
            task.getJvmArgs().add("-D" + clientGameTestEnabledProperty(project) + "=true");
    ...
    private static String clientGameTestEnabledProperty(Project project) {
        ClientGameTestRecorderExtension recorderExtension = project.getExtensions()
                .findByType(ClientGameTestRecorderExtension.class);
        return recorderExtension == null
                ? GameTestRecorderEnvironment.ENABLED_PROPERTY
                : recorderExtension.getEnabledProperty().get();
    }
