...
public final class WorkspaceDependenciesPlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        AtomicBoolean repositoriesConfigured = new AtomicBoolean();
        Runnable configureRepositories = () -> {
            if (repositoriesConfigured.compareAndSet(false, true)) {
                configureRepositories(project, extension);
            }
        };

        project.getConfigurations().configureEach(configuration ->
                configuration.getIncoming().beforeResolve(_ -> configureRepositories.run()));
        project.afterEvaluate(_ -> configureRepositories.run());

    }

    private static void configureRepositories(Project project, WorkspaceDependenciesExtension extension) {
        ...
        for (WorkspaceDependencyDeclaration declaration : extension.getDeclarations()) {
            ...
            MavenArtifactRepository repository = repositories.withType(MavenArtifactRepository.class).stream()
                    .filter(candidate -> candidate.getUrl().equals(repositoryUri))
                    .findFirst()
                    .orElseGet(() -> repositories.maven(candidate -> {
                        candidate.setName(repositoryName(declaration.getName()));
                        candidate.setUrl(repositoryUri);
                    }));
            repository.content(content -> content.includeModule(coordinate.group(), coordinate.artifact()));
